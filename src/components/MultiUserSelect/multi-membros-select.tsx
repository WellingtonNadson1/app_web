...
export function ComboboxDemo({ items, selectedItems, setSelectedItems }: ComboboxDemoProps) {
              ...
              {filteredItems.map((user) => (
                ...
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      selectedItems.some((selected) => selected.id === user.id) ? "opacity-100" : "opacity-0"
                    )}
                  />
                  {/* Tratamento para quando first_name for undefined */}

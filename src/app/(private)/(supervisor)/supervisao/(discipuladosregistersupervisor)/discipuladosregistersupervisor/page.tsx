'use client';
import Scheleton from '@/app/(private)/(celula)/celula/(discipuladosregisterliderelula)/components/listMembersCelulaDiscipulado/scheleton';
...
import FormSecondDiscipuladoSupervisor from '../components/listMembersSupervisorDiscipulado/FormSecondDiscipuladoSupervisor';
import FormFirstDiscipuladoSupervisor from '../components/listMembersSupervisorDiscipulado/FormFirstDiscipuladoSupervisor';
import {
  dataSchemaGetDiscipuladoAllSupervisor,
  dataSchemaReturnExistDiscipuladoAllCell,
  Supervisor,
} from '../components/listMembersSupervisorDiscipulado/schema';
import HeaderSupervisao from './HeaderDiscipuladosSupervisor';

export default function ListMembersSupervisorDiscipulado() {
  ...
  const { data: supervisorData } = useQuery<Supervisor>({
    queryKey: ['supervisorData'],
    enabled: status === 'authenticated',
    queryFn: async () => {
      const response = await axiosAuth.get(URLUser);
  ...
  const supervisor_id = supervisorData?.id as string;
    ...
    <Fragment>
      <div className="py-2 px-1">
        <div className="relative w-full mx-auto">
          <HeaderSupervisao
            headerSupervisao={supervisorData?.supervisao_pertence?.nome}
          />
        </div>
      </div>
      <div className="px-3">
            ...
            <div className="w-full px-1 py-2 rounded-md">
              ...
              <Pagination
                ...
                totalPages={Math.ceil(
                  (supervisorData?.discipulos?.length || 0) / itemsPerPage,
                )}

...
export async function POST(request: Request) {
  try {
    ...
    if (!pdfFile || pdfFile === null) {
      return NextResponse.json({ message: "PDF of lesson is required" }, { status: 400 });
    }
    ...
    const folderName = folderKeyNormalize.endsWith('/') ? folderKeyNormalize : `${folderKeyNormalize}/`;
    ...
    return NextResponse.json({ message: `Licao '${titulo}' created successfully!` }, { status: 200 });
  } catch (error) {

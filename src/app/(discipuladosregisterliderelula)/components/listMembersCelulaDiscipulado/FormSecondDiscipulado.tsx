"use client";
import { dataSchemaCreateDiscipulado, dataSchemaReturnCreateDiscipulado } from "@/app/(discipuladosregistersupervisor)/components/listMembersSupervisorDiscipulado/schema";
...
export default function FormSecondDiscipulado({ membro }: PropsForm) {
  ...
  const discipulador_id =
    membro?.discipulador[0]?.user_discipulador?.id;
  ...
  const isRegistered = Boolean(membro?.discipulador[0]?.discipulado[1]?.data_ocorreu);
